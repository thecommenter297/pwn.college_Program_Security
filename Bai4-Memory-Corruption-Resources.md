...
## Module 1: Lỗi Bộ Nhớ - Nền Tảng Của Mọi Cuộc Tấn Công
...
## Module 2: Bản Chất Của Lỗ Hổng - Khi Thiết Kế Trở Thành Vũ Khí
...
Trong các ngôn ngữ bậc cao như Python, trình thông dịch luôn duy trì siêu dữ liệu về kích thước của các cấu trúc dữ liệu. Khi bạn cố gắng truy cập mảng quá giới hạn:
```python
a = [ 1, 2, 3 ]
a[10] = 0x41 # IndexError: list index out of range
```
Chương trình lập tức dừng lại (Crash an toàn). Nhưng trong C, mọi thứ chỉ là các con trỏ và phép toán số học (pointer arithmetic). Trình biên dịch tin tưởng tuyệt đối vào độ chính xác của lập trình viên:

```c
int a[3] = { 1, 2, 3 };
a[10] = 0x41; // Không có cảnh báo nào!
```

Tại sao C lại cho phép điều này? Bởi vì việc chèn thêm các lệnh kiểm tra biên (bounds checking) trước mỗi thao tác truy cập mảng sẽ tiêu tốn chu kỳ CPU. Thay vào đó, thao tác `a[10]` chỉ đơn giản được biên dịch thành việc ghi dữ liệu vào địa chỉ `Base_Address + (10 * Kích_thước_phần_tử)`. 

Khi chương trình chạy, dữ liệu do người dùng kiểm soát (User Data) đi vào bộ nhớ thông qua các hàm như `read()`, `recv()`. Nếu không có cơ chế kiểm tra biên chặt chẽ, dữ liệu này sẽ âm thầm lan rộng (propagate) và đè lên các vùng nhớ lân cận. Đây chính là tiền đề của các lỗ hổng **OOB Read / OOB Write** – những mũi nhọn chủ lực trong việc săn 0-day trên các mục tiêu phức tạp như Trình duyệt web (V8/SpiderMonkey) hay Kernel.
...
## Module 3: Smashing the Stack - "Đập Vỡ" Ngăn Xếp
...
## Module 4: Cội Nguồn Của Sự Sụp Đổ - Phân Tích Logic Gây Lỗi
...
## Module 5: Lớp Phòng Thủ Đầu Tiên & Nghệ Thuật Rò Rỉ Dữ Liệu
...
## Module 6: Cuộc Chiến Bảng Phân Giải - Chiếm Đoạt GOT (Global Offset Table)
